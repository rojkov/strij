...
- [x] 5.1 Update `gateway.yaml` / `nodeagent.yaml` examples with `scheduler`, `pools`, `reservations`, `heartbeat_interval`, and `capacity` inside the task handler extension configs.
- [x] 5.2 End-to-end: one gateway (static discovery) + two nodeagents running `echo` with configured pools; verify routing, state snapshots, and rejection when a pool is exhausted.
- [x] 5.3 Run `make build`, `make test`, `make test_asan`, and `make clang-tidy`; fix findings.
