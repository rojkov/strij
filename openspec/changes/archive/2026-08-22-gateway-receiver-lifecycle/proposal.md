...
- `Node` gains a reference to `ResultReceiverStorage`. On connection creation it registers a mailbox close callback that calls `storage.NotifyNodeDisconnected(node_id)`. When a node connection drops, all orphaned receivers for that node are cleaned up in one shot.
...
- **`ResultReceiverStorage`** (`src/core/gateway/result_receiver_storage.hh`, `result_receiver_storage.cc`): new `node_of_task_` map, `NotifyNodeDisconnected()` and `NotifyClientDisconnected()` methods, modified `Put()` to accept and store `node_id`.
- **`GatewayHttpHandler`** (`src/core/gateway/gateway_http_handler.cc`): registers mailbox close callback in `HandleMessage()` that routes through `storage.NotifyClientDisconnected()`.
- **`Node`** (`src/core/gateway/node.hh`, `node.cc`): gains a `storage_` reference, registers close callback in `HandleCompletion(kConnect)`.
- **`NodeDirectory` / `gateway.cc`**: `Node` construction passes the storage reference.
