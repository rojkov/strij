...
- **Nodeagent: node-local-scheduler extension category.** Nodeagent config accepts a scheduler list; each entry yields one local scheduler instance (one per protocol). Initial "push" local scheduler preserves current behavior exactly: `kTaskSubmission` → admission → task handler → result/reject. `NodeagentTlvHandler` becomes a thin frame dispatcher (core frames → core, protocol frames → the local scheduler owning the type-id). The execution path is extracted into a core-owned `RunTask` service (parse → handler lookup → admission → run → result/reject). Nodeagent schedulers additionally implement `event::CommandHandler` for node-internal events; the role is defined now, first consumer lands in a later phase. `Schedule` facet reserved for child tasks.
...
- **Config schema** changes to match: `GatewayConfig.schedulers` (repeated, type-matched) and `NodeAgentConfig.schedulers` (repeated, at least one required) — both as plain breaking changes.
...
- `nodeagent-config`: gains a `repeated schedulers` list (at least one required) naming the node's local schedulers.
