...
`http_archive("jq", ...)` in `MODULE.bazel` (jq 1.8.2, sha256 `71b8d6e8f5fe81f6c6d0d110e3892251f6ce76ed095abd315e26e6e1193af3af`), plus `configure_make(name = "libjq", configure_in_place = True, out_static_libs = ["libjq.a"], lib_source = "@jq//:all_srcs")` in root `BUILD.bazel` (the `//:liburing` shape — see `src/common/core/event/BUILD.bazel` line 9 for consumption). Configure flags: `--disable-maintainer-mode` (skip autotools regeneration — the release tarball ships generated `parser.c`/`lexer.c`/`configure`, so bison/flex are not needed) and `--with-oniguruma=builtin` (use the oniguruma vendored in-tree since 1.8 instead of a system dep). Consumers include `#include "jq.h"` / `#include "jv.h"` exactly like the existing `#include "liburing.h"`.
...
`args` is positionally aligned with `variable_names` from `Compile`. This is a faithful mapping of libjq: `jq_state` IS the compiled program — `jq_compile_args` produces it, `jq_start_args`/`jq_next` drive it, `jq_get_error_message` reads failures, output is jq's linked-output chain (`jv` values → `vector<Jv>`). **All** outputs are returned; generator policy is a change-2 concern. `EvaluatorDeps` is an empty bundle for v1 (jq needs nothing from the framework) — passing `const&` keeps the factory signature uniform with `NodeSchedulerDeps`/`DataDependencyFetcherDeps` and leaves room to grow (e.g. a dispatcher ref) without breaking callers.
