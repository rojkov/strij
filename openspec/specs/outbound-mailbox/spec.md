...
Describes the connection-owned async outbound queue abstraction: an `OutboundMailbox` owned by `Connection` that queues frames to the connection's outbound write queue, becomes inert after close, and notifies registered close callbacks on teardown.
