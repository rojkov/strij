# data-dependency-fetcher

## Purpose

Defines the nodeagent data dependency prefetch subsystem: the `DataRef` wire message, the `Task.deps`/`TaskProbe.deps` fields, the `DataDependencyFetcher` extension interface and its scheme router, the node-global `ObjectCache`, the `DEP_COMPLETED` command, prefetch-on-enqueue behavior in the probe local scheduler, and fetcher configuration.
