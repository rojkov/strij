...
Defines the probe scheduling protocol: how the gateway selects candidate nodes and arbitrates concurrent pulls, how the node agent handles probe arrival, deferred admission, and reservation lifecycle, and the deadline/resolution guarantees that ensure every task handed to a `"probe"` scheduler resolves.
