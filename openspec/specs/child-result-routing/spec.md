...
Defines how child-task outcomes are routed back to the parent on the node side and across the gateway: the node-local receiver storage, the router-mediated inbound outcome frames, the receiver-backed result sender for local children, the gateway node-connection result receiver, and the two-hop delivery path for forwarded children.
