...
Defines how HTTP request headers are forwarded into the `Task.parameters` map so task handlers can receive per-task metadata (e.g. which executable to run) alongside the task body.
