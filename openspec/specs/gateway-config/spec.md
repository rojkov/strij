...
message GatewayConfig {
  ...
  TlsConfig tls = 6;
  ExtensionConfig node_discovery = 7 [(strij.config.required) = true];
  repeated SchedulerConfig schedulers = 8 [(strij.config.required) = true];
}

message SchedulerConfig {
  ExtensionConfig extension = 1 [(strij.config.required) = true];
  string task_type = 2;  // empty = default scheduler for unmatched types
}
