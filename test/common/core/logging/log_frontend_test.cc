...
#include "common/core/logging/log_frontend.hh"
...
TEST(LogFrontendTest, pack_args_trivially_copyable) {
  LogEntry entry{};
  std::byte* ptr = entry.args_data_.data();
  ...
  ASSERT_EQ(*(uint32_t*)entry.args_data_.data(), first_arg);
  ASSERT_EQ(*(uint64_t*)(std::next(entry.args_data_.data(), sizeof(first_arg))), second_arg);
  ASSERT_EQ(
      *(uint32_t*)(std::next(entry.args_data_.data(), sizeof(first_arg) + sizeof(second_arg))),
      third_arg);
  ASSERT_EQ(std::string{(char*)(entry.args_data_.data() + sizeof(first_arg) + sizeof(second_arg) +
                                sizeof(third_arg))},
            "test");
  ASSERT_EQ(
      *(uint32_t*)(std::next(entry.args_data_.data(), sizeof(first_arg) + sizeof(second_arg) +
...
TEST(LogFrontendTest, pack_args_string_view) {
  ...
  ASSERT_EQ(*(size_t*)entry.args_data_.data(), test_str.size());
  char str_data[512]{};
  std::memcpy(&str_data, entry.args_data_.data() + sizeof(size_t), test_str.size());
  ASSERT_EQ(std::string_view{(char*)str_data}, test_str);
  ASSERT_EQ(*(uint32_t*)(entry.args_data_.data() + sizeof(size_t) + test_str.size()), uint32_t(10));
}
