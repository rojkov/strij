...
auto make_frame(uint8_t type, std::span<const std::byte> value) -> std::vector<std::byte> {
...
protected:
  ...
  TlvParser parser_{[this](std::span<const std::byte> msg) -> void {
    received_.emplace_back(msg.begin(), msg.end());
  }};
...
TEST_F(TlvParserTest, SingleCompleteFrame) {
  ...
  feed(make_frame(0x01, value));
  ASSERT_EQ(received_.size(), 1U);
  EXPECT_EQ(received_[0], value);
}
...
TEST_F(TlvParserTest, MultipleFramesInOneRead) {
  ...
  auto frame1 = make_frame(0x01, value1);
  auto frame2 = make_frame(0x02, value2);
  ...
  EXPECT_EQ(received_[0], value1);
  EXPECT_EQ(received_[1], value2);
}

TEST_F(TlvParserTest, ZeroLengthValue) {
  feed(make_frame(0x01, std::span<const std::byte>{}));
  ASSERT_EQ(received_.size(), 1U);
...
TEST_F(TlvParserTest, LengthFieldSplitAcrossReads) {
  ...
  auto frame = make_frame(0x01, value);
...
TEST_F(TlvParserTest, BufferCompaction) {
  ...
  auto frame1 = make_frame(0x01, big_value);
  ASSERT_EQ(frame1.size(), 4094U);

  feed(frame1);
  ASSERT_EQ(received_.size(), 1U);
  EXPECT_EQ(received_[0], big_value);
...
TEST_F(TlvParserTest, MultipleFramesByteByByte) {
  ...
  auto frame1 = make_frame(0x10, value1);
  auto frame2 = make_frame(0x20, value2);
