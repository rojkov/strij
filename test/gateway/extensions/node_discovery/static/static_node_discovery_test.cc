...
#include "gateway/extensions/node_discovery/static/static_node_discovery.hh"
...
protected:
  auto CollectNodes(std::unique_ptr<NodeDiscovery> discovery) -> std::vector<gateway::NodeInfo> {
    std::vector<gateway::NodeInfo> collected;
    discovery->Start(
        [&collected](std::vector<gateway::NodeInfo> nodes) { collected = std::move(nodes); });
    return collected;
  }
};
