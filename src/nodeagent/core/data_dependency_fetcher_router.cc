...
auto BuildDataDependencyFetchers(
  ...
  for (const auto& ext : configs) {
    auto* registry = &extensions::Registry<nodeagent::DataDependencyFetcherFactory>::instance();
    ...
    if (factory == nullptr) {
      ...
      return absl::InvalidArgumentError(absl::StrCat(
          "Data dependency fetcher '", ext.name(),
          "' not found. Registered: ", names.empty() ? "(none)" : absl::StrJoin(names, ", ")));
    }
    ...
    if (!unpacked.type_url().empty() && !unpacked.UnpackTo(config_msg.get())) {
      return absl::InvalidArgumentError(
          absl::StrCat("Failed to unpack typed_config for data dependency fetcher '", ext.name(),
...
auto DataDependencyFetcherRouter::Build(nodeagent::ObjectCache& cache,
                                        std::vector<extensions::DataDependencyFetcherPtr> fetchers)
...
auto DataDependencyFetcherRouter::AllCached(
  ...
  return std::ranges::all_of(deps, [this](const task::DataRef& ref) -> bool {
    ...
    return !by_source_.contains(ref.source()) || cache_.IsCached(ref);
  });
}
