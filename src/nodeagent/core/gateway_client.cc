...
auto GatewayClient::Forward(const task::Task& task) -> absl::Status {
  ...
  live_mailboxes_.erase(
      std::remove_if(live_mailboxes_.begin(), live_mailboxes_.end(),
                     [](io::OutboundMailbox* m) { return m == nullptr; }),
      live_mailboxes_.end());

  if (live_mailboxes_.empty()) {
    return absl::UnavailableError("no live gateway connection to forward child task '" +
                                  task.id() + "'");
  }
...
void GatewayClient::RegisterConnection(std::shared_ptr<io::OutboundMailbox> mailbox) {
  ...
  mailbox->RegisterOnClose([this, mb = std::move(mailbox)]() {
    auto it = std::find(live_mailboxes_.begin(), live_mailboxes_.end(), mb.get());
    if (it != live_mailboxes_.end()) {
      *it = nullptr;
    }
  });
}
