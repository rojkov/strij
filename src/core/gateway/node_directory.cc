...
void NodeDirectory::AddNode(std::string node_id, std::string address) {
...
void NodeDirectory::Reconcile(const std::vector<strij::extensions::NodeInfo>& snapshot) {
  ...
  for (const auto& info : snapshot) {
    ...
    auto origin = origin_to_canonical_.find(info.node_id);
    const std::string& key = origin != origin_to_canonical_.end() ? origin->second : info.node_id;
    present_ids.insert(key);

    auto it = nodes_.find(key);
    if (it == nodes_.end()) {
      AddNode(info.node_id, info.address);
    } else if (it->second->GetAddress() != info.address) {
      LOG_INFO("Node {} re-announced at new address {}; reconnecting", key, info.address);
      it->second->ReconnectTo(info.address);
    }
  }
...
void NodeDirectory::RekeyNode(const std::string& from_id, const std::string& to_id) {
  ...
  it->second->SetNodeId(to_id);
  nodes_.emplace(to_id, std::move(it->second));
  nodes_.erase(it);
}
...
auto NodeDirectory::GetNextNode() -> Node* {
  ...
  for (size_t i = 0; i < total; ++i) {
    ...
    std::advance(it, static_cast<ssize_t>(idx));
...
auto NodeDirectory::GetCandidates(std::string_view protocol) -> std::vector<Node*> {
  ...
  for (const auto& [node_id, node] : nodes_) {
    ...
    const bool advertises_protocol = std::any_of(
        ...
        [protocol](const auto& entry) { return entry.name() == protocol; });
