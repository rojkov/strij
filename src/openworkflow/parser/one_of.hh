...
inline auto SelectOne(const YAML::Node& node, KeySet keys, std::string_view what) -> std::string_view {
    ...
    if (count == 0) {
        ...
        throw ParseError(node.Mark(),
                         "expected exactly one " + std::string(what) + " key but found none among " +
                             expected);
    }
    if (count > 1) {
