...
- **Extensions:** `Registry<FactoryInterface>` singleton template. `REGISTER_FACTORY(FactoryClass, FactoryInterface)` macro for simple names; `REGISTER_FACTORY_FULLY_QUALIFIED(FactoryClass, FactoryInterface, RegistrarName)` for namespace-qualified names. Factory contexts are split by side: `FactoryContext` base provides `Dispatcher()` and `Logger()`; `GatewayFactoryContext` adds `NodeDirectory()` and `ResultReceiverStorage()`; `NodeagentFactoryContext` adds `FunctionResolver()`, `AdmissionController()`, and `RunTask()`. Concrete impls (`GatewayFactoryContextImpl`, `NodeagentFactoryContextImpl`) live in `src/exe/*` and are compiled inline in the binaries (set `include_prefix`-less paths). Extension categories: `NodeDiscovery` / `NodeDiscoveryFactory` (node discovery plugins; `StaticNodeDiscovery` uses a hardcoded address list), task handlers (`ResultSender`, `TaskHandler` / `TaskHandlerFactory`, `EchoTaskHandler` in `extensions/task_handlers/echo`; loaded from `NodeAgentConfig.task_handlers` via `BuildTaskHandlerManager`), and schedulers (`GatewaySchedulerFactory` / `NodeSchedulerFactory`, registered implementations under `src/extensions/schedulers/{round_robin,capability_aware,push}`, composite routing in `scheduler_router_lib`).
