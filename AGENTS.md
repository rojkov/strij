...
- **Layout:** side-first tree mirroring the side split, with shared scaffolding in `common` and the curated public API under `include/`: sources `src/{common,gateway,nodeagent}/{core,extensions,exe}` (e.g. `src/common/core/{event,io,logging,config,utils,common}`, `src/common/extensions/{registry,factory_context,scheduler}`, `src/gateway/core/{node_directory,result_receiver_storage,scheduler_router,...}`, `src/nodeagent/core/{admission_controller,run_task_service,task_handler_manager,function_resolver,...}`); protos `api/common/{config,task,node}`, `api/gateway/...`, `api/nodeagent/...`; tests `test/{common,gateway,nodeagent}/...` mirroring `src/`; shared mocks `test/mocks/`.
